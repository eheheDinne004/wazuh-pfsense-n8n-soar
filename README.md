...
```text
[Thiết bị WiFi / Kẻ tấn công]
             │
             ▼ (Gửi gói tin / Xin cấp IP)
     [pfSense Gateway]
             │
             ▼ (Đẩy Syslog)
       [Wazuh SIEM] ──(Nhận diện: Nmap, Rogue MAC, Brute Force)
             │
             ▼ (Gửi Webhook JSON nếu Level >= 5)
        [n8n SOAR]
             ├────────────────────────┬────────────────────────┐
             ▼                        ▼                        ▼
  [Thực thi SSH Block IP]   [Thông báo Telegram]     [Ghi Log Google Sheets]
    (Chặn trên pfSense)    (Cảnh báo tức thì)        (Lưu vết truy xuất)
